...
angular.module('pinApp')
	.controller('NavbarCtrl', ['$scope', '$location', '$rootScope', 'Auth',
		'$http', '$modal',
					...
					Auth.login({
						...
						.then(function() {
							...
							if ($rootScope.currentUser.role == 'admin') {
								...
								if ($rootScope.redirectPath && $rootScope.redirectPath !==
									'/register') {
						...
						.catch(function(err) {
							...
							$location.path('/login').search({
								'loginError': err.message.message,
								'field': err.message.field
							});
...
angular.module('pinApp')
	.controller('NavLoginCtrl', function($scope, Auth, $location, $rootScope,
		$modalInstance) {
		$scope.user = {};
		$scope.errors = {};
